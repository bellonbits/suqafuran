...
# ===== PYDANTIC MODELS FOR SHOP REVIEWS =====
...
def get_shop_reviews(shop_id: str = Path(...), db: Session = Depends(get_db)):
    try:
        ...
        return {
            "reviews": [{"id": r.id, "display_name": r.display_name or "Anonymous", "rating": r.rating, "review": r.review, "is_verified_purchase": r.is_verified_purchase, "created_at": r.created_at.isoformat() if r.created_at else None} for r in reviews],
            "average_rating": float(avg) if avg else 0,
            "total_reviews": total or 0,
            "verified_reviews_count": verified or 0
        }
    except:
        return {"reviews": [], "average_rating": 0, "total_reviews": 0, "verified_reviews_count": 0}

@listings_router.post("/shops/{shop_id}/reviews")
def post_shop_review(shop_id: str = Path(...), data: ReviewCreate = Body(...), db: Session = Depends(get_db)):
    try:
...
def post_shop_feedback(shop_id: str = Path(...), data: FeedbackCreate = Body(...), db: Session = Depends(get_db)):
    try:
