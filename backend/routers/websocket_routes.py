...
async def notifications_websocket(
    ...
    Usage:
    1. Connect: ws://localhost:8000/api/v1/notifications/ws?token={jwt_token}
    2. Receive notifications: {"type": "notification", "title": "...", "message": "..."}
    """
    # Verify JWT token and get user
    try:
        from jose import jwt
        ...
        logger.error(f"Notifications WebSocket auth error: {str(e)}")
        await websocket.close(code=4003, reason="Authentication failed")
        return
    ...
    try:
        ...
        logger.info(f"User {user_id} connected to notifications: {connection_id}")

        # Subscribe to personal notifications channel
        await manager.subscribe(connection_id, f"notifications_{user_id}")
        await manager.subscribe(connection_id, "notifications_broadcast")  # For admin broadcasts
        ...
        logger.info(f"Notifications WebSocket disconnect: {user_id}:{connection_id}")
