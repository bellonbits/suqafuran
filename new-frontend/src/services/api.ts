...
export const API_BASE_URL =
  ...
  'https://app.suqafuran.com/api/v1';

const API_ORIGIN = typeof window !== 'undefined'
  ? window.location.origin
  : (process.env.NEXT_PUBLIC_API_URL?.replace(/\/api\/v1$/, '') || 'https://app.suqafuran.com');
...
api.interceptors.request.use(
    (config) => {
        ...
        // Add headers to bypass Cloudflare bot protection for mobile apps
