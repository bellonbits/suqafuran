...
import { LaunchPromotion } from '@/components/seller/LaunchPromotion';
  ...
  return (
    <div className="space-y-8">
      ...
      {showPromo && user && (
        <LaunchPromotion
          sellerId={user.id}
          onStart={() => router.push('/seller-dashboard/subscription')}
      ...
      {user && (
        <div>
          <h2 className="text-lg font-black text-gray-900 dark:text-white mb-4">Your Plan</h2>
          <SubscriptionCard
            sellerId={user.id}
            onUpgradeClick={() => router.push('/seller-dashboard/subscription')}
          />
        </div>
      )}
