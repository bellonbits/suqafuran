...
export default function EditShopPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, fileType: 'logo' | 'shop_page_banner' | 'shop_detail_banner') => {
    ...
    reader.onloadend = () => {
      setPreviews({ ...previews, [fileType]: reader.result as string });
    };
  ...
  const handleSave = async () => {
    try {
      ...
      // Upload logo to Cloudinary first if it's a file
      if (formData.logo) {
        await uploadBannerToCloudinary(formData.logo, 'logo');
      }
      ...
      // Prepare update data (without files since they're uploaded separately)
  ...
  const uploadBannerToCloudinary = async (
    file: File,
    fileType: 'logo' | 'shop_page_banner' | 'shop_detail_banner',
    retries = 3
  ) => {
    let lastError: any;
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        console.log(
          `📤 Uploading ${fileType} to Cloudinary... (attempt ${attempt}/${retries})`
        );
        ...
        const folder = fileType === 'logo' ? 'suqafuran' : 'suqafuran/banners';
        ...
        console.log(`✅ ${fileType} uploaded to Cloudinary:`, imageUrl);

        // Map fileType to database field name
        const dbFieldMap = {
          logo: 'logo_url',
          shop_page_banner: 'shop_page_banner',
          shop_detail_banner: 'shop_detail_banner',
        };
        const dbFieldName = dbFieldMap[fileType];

        // Now send ONLY the URL to backend to save to database
        const response = await api.put(
          `/admin/shops/${shopId}`,
          { [dbFieldName]: imageUrl }
        );

        console.log(`✅ ${fileType} saved to database`);
        setPreviews({ ...previews, [fileType]: imageUrl });
        return;
      } catch (error: any) {
        lastError = error;
        console.warn(
          `⚠️ Attempt ${attempt} failed for ${fileType}:`,
          error.message
        );
    ...
    // All retries failed
    throw new Error(
      `Failed to upload ${fileType} after ${retries} attempts: ${lastError.message}`
    );
  };
  ...
  const handleDeleteBanner = async (fileType: 'logo' | 'shop_page_banner' | 'shop_detail_banner') => {
    const labels = {
      logo: 'Shop Logo',
      shop_page_banner: 'Card Banner',
      shop_detail_banner: 'Detail Banner',
    };
    if (!confirm(`Delete ${labels[fileType]}?`)) return;

    try {
      const endpoint = fileType === 'logo'
        ? `/admin/shops/${shopId}/logo`
        : `/admin/shops/${shopId}/banner/${fileType === 'shop_page_banner' ? 'shop_page' : 'shop_detail'}`;

      await api.delete(endpoint);
      setPreviews({ ...previews, [fileType]: '' });
      alert(`${labels[fileType]} deleted successfully`);
      loadShop();
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow p-8 space-y-8">
          ...
          <section className="border-t pt-8">
            <h2 className="text-xl font-bold text-gray-900 mb-6">Shop Logo</h2>
            <p className="text-sm text-gray-600 mb-4">Used on shop cards and detail pages. Recommended: 500 × 500 px (square)</p>
            ...
            {previews.logo && (
              <div className="mb-6">
                <div className="relative w-32 h-32 bg-gray-100 rounded-lg overflow-hidden">
                  ...
                  />
                  <button
                    onClick={() => handleDeleteBanner('logo')}
