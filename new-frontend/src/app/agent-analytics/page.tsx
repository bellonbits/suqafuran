...
import { BarChart3, TrendingUp, ArrowLeft, Loader } from 'lucide-react';
...
export default function AgentAnalyticsPage() {
  ...
  const loadConversions = async () => {
    try {
      const res = await api.get('/promotions/agent/conversions');
  ...
  const metrics = [
    { label: 'Total Listings', value: conversions?.total_listings?.toString() || '0', trend: 'up', percent: 23 },
    { label: 'Total Views', value: conversions?.total_views?.toString() || '0', trend: 'up', percent: 18 },
    { label: 'Conversion Rate', value: `${conversions?.conversion_rate || 0}%`, trend: 'up', percent: 8 },
    { label: 'Avg Days to Sale', value: `${conversions?.avg_days_to_sale || 0}d`, trend: 'down', percent: 5 },
  ];
