...
export default function GroceryPage() {
    ...
    useEffect(() => {
        async function loadGroceryData() {
            try {
                const fetchedListings = await listingsService.getListings({ category_id: 'food-groceries' });
    ...
    groceryListings.forEach(l => {
        const mappedProduct: DealProduct = {
            id: l.id,
            name: l.title_en || l.title_so || "Grocery Item",
            image: l.images?.[0] || "https://images.unsplash.com/photo-1542838132-92c53300491e?q=80&w=200",
            price: l.price,
            originalPrice: Math.round(l.price * 1.25),
            badge: l.condition || "Fresh"
        };

        const subcategoryName = l.subcategory_id?.toString() || "";
