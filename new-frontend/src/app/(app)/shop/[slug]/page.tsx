...
import {
  ChevronRight, Star, MapPin, Edit, Settings, Phone, Mail, Clock, Heart, Share2, MessageCircle, MessageSquare,
...
export default function PublicShopPage() {
  ...
  // Track shop visit for analytics
  useEffect(() => {
    if (shopData?.user_id) {
  ...
  const fetchShopData = async () => {
    try {
      ...
      // Fetch from listings/shops with slug search (primary method that works)
      ...
      shopProfile = shop;
      console.log('Shop data from API:', shop);
      console.log('Logo URL from API:', shop.logo_url);
      const rawLogo = shop.logo_url || shop.owner_avatar_url || shop.cover_image;
      ...
      setShopData({
        ...
        email: shop.email || '',
        logo_url: logoUrl,
        banner_url: shop.banner_url || shop.shop_page_banner,
  ...
  const handleContact = (type: 'whatsapp' | 'phone' | 'email') => {
    const sellerId = shopData?.user_id ? Number(shopData.user_id) : null;

    if (type === 'whatsapp' && shopData?.phone) {
      if (sellerId) {
        sellerAnalyticsService.trackWhatsAppClick(sellerId);
      }
      window.open(`https://wa.me/${shopData.phone.replace(/\D/g, '')}`);
    } else if (type === 'phone' && shopData?.phone) {
      if (sellerId) {
        sellerAnalyticsService.trackCallClick(sellerId);
      }
      window.location.href = `tel:${shopData.phone}`;
    } else if (type === 'email' && shopData?.email) {
      if (sellerId) {
        sellerAnalyticsService.trackMessageClick(sellerId);
      }
