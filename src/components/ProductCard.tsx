        ...
        >
            <div className="relative aspect-[4/5] sm:aspect-[4/3] overflow-hidden bg-gray-100">
                ...
                <div className="absolute top-2 left-2 flex flex-col gap-1.5 pointer-events-none">
                    ...
                    {isVerified && (
                        <div className="bg-primary-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded shadow-sm w-fit">
                            Verified ID
                        </div>
                    )}
                </div>
            ...
            <div className="p-3 flex flex-col flex-1">
                ...
                <div className="grid grid-cols-2 border-t border-gray-50 h-10">
                    <button
                        onClick={async (e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            if (id) {
                                try {
                                    ...
                                    // Normally we would show the phone or initiate a call
                                    window.location.href = `tel:${id}`; // Placeholder, id is not the phone
                                } catch (err) {
                                    console.error('Failed to log call', err);
                                }
                            }
                        }}
                        className="flex items-center justify-center gap-1.5 hover:bg-primary-50 text-gray-700 hover:text-primary-600 font-bold transition-all border-r border-gray-50"
                    >
                        ...
                        onClick={async (e) => {
                            ...
                            if (id) {
                                try {
                                    ...
                                    // Placeholder for whatsapp link - we'd need the phone in props
                                } catch (err) {
                                    console.error('Failed to log whatsapp', err);
                                }
                            }
                        }}
                        className="flex items-center justify-center gap-1.5 hover:bg-green-50 text-gray-700 hover:text-green-600 font-bold transition-all"
                    >
