...
const ListingFormPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const catResponse = await axios.get('/api/v1/listings/categories');
        setCategories(catResponse.data);

        if (isEditing) {
          const listingResponse = await axios.get(`/api/v1/listings/${listingId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing) {
        await axios.put(`/api/v1/listings/${listingId}`, payload);
